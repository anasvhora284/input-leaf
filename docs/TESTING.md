...
The `android-coverage` CI job verifies on the API 36 emulator that `:app:createDebugCoverageReport` passes and runs the service and onboarding smoke tests plus the connected `ConnectionServiceLifecycleTest` suite, which produces the JaCoCo report Codecov uses for the `android` flag.
